...
package com.document.scanner.imageloading

import com.document.scanner.imageloading.utils.PicassoTarget
import com.document.scanner.imageloading.utils.PicassoTransformation
import com.document.scanner.imageloading.utils.into
...
interface ImageLoader {
    ...
    fun cancelRequests(destination: Config.Destination)
...
class ImageLoaderImpl @Inject constructor(
    ...
    override fun loadImage(config: Config) {
        ...
        when(config.destination) {
            is Config.Destination.View -> requestCreator.loadIntoImageView(config, config.destination)
            is Config.Destination.Callback -> requestCreator.loadIntoCallback(config, config.destination)
        }
    }


    private fun RequestCreator.loadIntoImageView(config: Config, destination: Config.Destination.View) {
    ...
    private fun RequestCreator.loadIntoCallback(config: Config, destination: Config.Destination.Callback) {
    ...
    private fun createPicassoTargetWithStrongReference(
        configKey: String,
        destination: Config.Destination.Callback
    ): PicassoTarget {
    ...
    override fun cancelRequests(destination: Config.Destination) {
        when(destination) {
            is Config.Destination.View -> picasso.cancelRequest(destination.imageView)
            is Config.Destination.Callback -> targetsMap.clear()
        }
    }
